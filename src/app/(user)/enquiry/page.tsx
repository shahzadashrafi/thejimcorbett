...
export default function SignupPage(){
    ...
    const onEnquiry = async ()=>{
        try {
            setLoading(true)
            const response = await axios.post('/api/enquiry', visitor)
            console.log('Enquiry Done!', response.data)
            router.push('/home')

        } catch (error: any) {
            console.log("Error in Enquiry")
    ...
    useEffect(()=>{
        if(visitor.name.length>0 && visitor.email.length>0 && visitor.mobile.length>0 && visitor.date.length>0){
    ...
    return(
        ...
        <div className="flex min-h-screen items-center justify-center">
            <div className="relative flex flex-col rounded-xl bg-transparent bg-clip-border text-gray-700 shadow-none">
                <h4 className="block font-sans text-2xl font-semibold leading-snug tracking-normal text-blue-gray-900 antialiased">Sign Up</h4>
                <p className="mt-1 block font-sans text-base font-normal leading-relaxed text-gray-700 antialiased">Enter your details to register.</p>
